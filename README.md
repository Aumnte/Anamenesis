[Ananmnesis.html](https://github.com/user-attachments/files/27354705/Ananmnesis.html)
<!DOCTYPE html>
<html lang="es">
<head>
<meta charset="UTF-8">
...
@media (max-width: 1024px) {
  /* Hacemos que los chips sean un poco más grandes para el dedo */
  .chip {
    padding: 10px 16px;
    font-size: 13px;
    margin-bottom: 4px;
  ...
  /* Los inputs y select deben tener una altura mínima de 44px (estándar de Apple) */
  input[type="text"], 
  input[type="number"], 
  select, 
  textarea {
  ...
  /* Ajustamos las rejillas para que no se amontonen en vertical */
  .grid3, .grid4 {
    grid-template-columns: 1fr 1fr; 
  }
  
  /* El menú de navegación debe ser más fácil de deslizar */
  .nav {
    padding: 5px 0;
    gap: 10px;
  }
  ...
  .nav-tab {
    padding: 15px 20px;
.body-svg-wrap {
  touch-action: none; /* Evita que el navegador haga scroll mientras marcas el dolor */
}
  }
...
canvas {
  ...
  background-size: 100% 40px; /* Líneas guía como un cuaderno */
}
}
...
<aside class="sidebar">
  ...
  <div class="sidebar-footer">
    ...
    <button class="btn-sidebar" onclick="window.print()">Imprimir</button>
    <button class="btn-sidebar primary" onclick="saveSession()">Guardar</button>
  </div>
</aside>
      ...
      <div class="body-chart-wrap">
        <div class="body-figure">
          ...
          <div class="body-canvas-wrap" id="bc-ant" onclick="addDot(event,'bc-ant')">
            <svg viewBox="0 0 120 280" xmlns="http://www.w3.org/2000/svg" style="pointer-events:none">
...
<div class="panel" id="p7">
  ...
  <div class="card">
    <div class="card-header"><div class="card-header-icon" style="background:var(--teal);color:#fff">📍</div><span class="card-title">Seleccionar zona</span></div>
    <div class="card-body">
      <div class="zone-tabs">
        <div class="zone-tab" onclick="showZone('mob','cervical',this)">Cervical</div>
        <div class="zone-tab" onclick="showZone('mob','lumbar',this)">Lumbar</div>
        <div class="zone-tab" onclick="showZone('mob','hombro',this)">Hombro</div>
        <div class="zone-tab" onclick="showZone('mob','codo',this)">Codo</div>
        <div class="zone-tab" onclick="showZone('mob','muneca',this)">Muñeca</div>
        <div class="zone-tab" onclick="showZone('mob','cadera',this)">Cadera</div>
        <div class="zone-tab" onclick="showZone('mob','rodilla',this)">Rodilla</div>
        <div class="zone-tab" onclick="showZone('mob','tobillo',this)">Tobillo / Pie</div>
      </div>
      <p style="font-size:12px;color:var(--ink-faint);font-family:var(--mono)">↑ Selecciona una región para mostrar los movimientos</p>
    </div>
  </div>
...
<div class="panel" id="p8">
  ...
  <div class="card">
    ...
    <div class="card-body">
      <div class="zone-tabs">
        <div class="zone-tab" onclick="showZone('str','mmss',this)">MMSS</div>
        <div class="zone-tab" onclick="showZone('str','mmii',this)">MMII</div>
        <div class="zone-tab" onclick="showZone('str','tronco',this)">Tronco / Core</div>
      </div>
    </div>
  </div>

  <div id="str-mmss" class="str-section">
    <div class="card"><div class="card-header"><div class="card-header-icon" style="background:var(--blue);color:#fff">💪</div><span class="card-title">Fuerza Miembro Superior — MRC</span></div><div class="card-body">
    <table class="data-table" id="t-mmss"><thead><tr><th>Músculo / Grupo</th><th class="center">MRC Derecho</th><th class="center">MRC Izquierdo</th><th>Notas</th></tr></thead><tbody id="b-mmss"></tbody></table>
    </div></div>
  </div>
  <div id="str-mmii" class="str-section">
    <div class="card"><div class="card-header"><div class="card-header-icon" style="background:var(--blue);color:#fff">💪</div><span class="card-title">Fuerza Miembro Inferior — MRC</span></div><div class="card-body">
    <table class="data-table" id="t-mmii"><thead><tr><th>Músculo / Grupo</th><th class="center">MRC Derecho</th><th class="center">MRC Izquierdo</th><th>Notas</th></tr></thead><tbody id="b-mmii"></tbody></table>
    </div></div>
  </div>
  <div id="str-tronco" class="str-section">
    <div class="card"><div class="card-header"><div class="card-header-icon" style="background:var(--blue);color:#fff">💪</div><span class="card-title">Fuerza Tronco / Core — MRC</span></div><div class="card-body">
    <table class="data-table" id="t-tronco"><thead><tr><th>Músculo / Grupo</th><th class="center">MRC</th><th>Notas</th></tr></thead><tbody id="b-tronco"></tbody></table>
    </div></div>
  </div>
...
const TITLES = [
  ...
  'Valoración Neurológica','Tests Ortopédicos','Razonamiento Clínico'
];
let current = 0;
...
function saveAllData() {
  ...
  // Guardar Chips seleccionados (por texto para identificar)
  const selectedChips = [];
  document.querySelectorAll('.chip.selected').forEach(chip => {
    selectedChips.push({
      text: chip.innerText,
      groupId: chip.parentElement.id
    });
  });
  data.chips = selectedChips;

  // Guardar puntos del Body Chart
  const dots = [];
  document.querySelectorAll('.pain-dot').forEach(dot => {
    dots.push({
      ...
      parent: dot.parentElement.id
    });
  });
  data.dots = dots;

  localStorage.setItem('physioFormData', JSON.stringify(data));
}

// 2. Cargar datos al abrir
function loadData() {
  const rawData = localStorage.getItem('physioFormData');
  if (!rawData) return;
  ...
  // Restaurar Chips
  if (data.chips) {
    data.chips.forEach(cData => {
      const group = document.getElementById(cData.groupId);
      if (group) {
        const chips = group.querySelectorAll('.chip');
        chips.forEach(chip => {
          if (chip.innerText === cData.text) chip.classList.add('selected');
        });
      }
    });
  }

  // Restaurar Puntos del Body Chart
  if (data.dots) {
    data.dots.forEach(d => {
      const parent = document.getElementById(d.parent);
      if (parent) {
...
// --- MODIFICACIONES EN TUS FUNCIONES EXISTENTES ---

// Al final de tu script, añade esto para que cargue al iniciar:
document.addEventListener('DOMContentLoaded', loadData);

// Modifica tu updateProgress para que guarde cada vez que hay un cambio:
function updateProgress() {
  // ... (tu código actual de updateProgress) ...
  saveAllData(); // <--- Añade esto al final de la función
}

// Modifica clearAll para limpiar también el almacenamiento:
function clearAll() {
  if (!confirm('¿Limpiar toda la anamnesis?')) return;
  localStorage.removeItem('physioFormData'); // <--- Añade esto
  location.reload(); // Recarga para limpiar todo visualmente de golpe
}
// --- LÓGICA DE FIRMA ---
...
// Modifica tu saveAllData para incluir:
// data.signature = canvas.toDataURL();
// data.photos = Array.from(previewContainer.querySelectorAll('img')).map(i => i.src);

// Modifica tu loadData para restaurar:
// if(data.signature) { 
//    const img = new Image(); img.src = data.signature; 
...
function addDot(e, chartId) {
  ...
  const dot = document.createElement('div');
  dot.className = 'pain-dot';
  dot.style.cssText = `left:${x}%;top:${y}%;width:13px;height:13px;background:${currentColor};`;
...
function buildMrcTable(bodyId, muscles, hasSides) {
  const tbody = document.getElementById(bodyId);
  if(!tbody) return;
  tbody.innerHTML = muscles.map((m,i) => {
    const row = bodyId + i;
    if(hasSides) {
      return `<tr><td>${m}</td><td style="text-align:center">${mrcCell(row+'D')}</td><td style="text-align:center">${mrcCell(row+'I')}</td><td><input type="text" style="width:100%;font-family:var(--sans);font-size:12px;padding:4px 8px;border:1.5px solid var(--border);border-radius:5px;outline:none" placeholder="obs..."></td></tr>`;
    } else {
      return `<tr><td>${m}</td><td style="text-align:center">${mrcCell(row)}</td><td><input type="text" style="width:100%;font-family:var(--sans);font-size:12px;padding:4px 8px;border:1.5px solid var(--border);border-radius:5px;outline:none" placeholder="obs..."></td></tr>`;
    }
  }).join('');
}
buildMrcTable('b-mmss', mmssData, true);
buildMrcTable('b-mmii', mmiiData, true);
buildMrcTable('b-tronco', troncoData, false);

// ── SAVE / CLEAR ─────────────────────────────────────────────────────────────
function saveSession() { window.print(); }
function clearAll() {
...
<!-- SECCIÓN DE CIERRE: FOTOS Y FIRMA -->
<div class="card" style="margin-top:20px; border-top: 3px solid var(--accent2);">
  <div class="card-title">Documentación Visual y Firma</div>
  
  <div class="grid2">
    <!-- Captura de Fotos -->
    <div class="field">
      <label>Capturas (Postura, RMN, Eco...)</label>
      <input type="file" id="foto-input" accept="image/*" capture="environment" style="margin-bottom:10px">
      <div id="preview-container" style="display:flex; gap:10px; flex-wrap:wrap;"></div>
    </div>

    <!-- Firma Digital -->
    <div class="field">
      <label>Firma del Paciente / Consentimiento</label>
      <div style="border:1px solid var(--border); background:white; border-radius:4px; position:relative;">
        <canvas id="signature-pad" width="400" height="200" style="touch-action: none; width:100%; cursor:crosshair;"></canvas>
        <button class="tool-btn" onclick="clearSignature()" style="position:absolute; top:5px; right:5px;">Borrar Firma</button>
      </div>
    </div>
  </div>
</div>
